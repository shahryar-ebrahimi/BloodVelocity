...
signal    = load('EX1_carotis.mat'); signal = signal.data;
...
depth = 1000*(c/2)*(2e-6:2e-6:floor((size(signal, 1))/200)*2e-6);   % depth (mm)
plot(depth, Blood_v); title('Blood Velocity vs Depth'); ylabel('Blood Speed (m/s)'); xlabel('Depth (mm)');
